...
Original file is located at
    https://colab.research.google.com/drive/1G7CDeJ14TShjT48_sYW3zUL9Ui3jxwHa
"""
...
# For visualizations using seaborn in later cells
...
def estimate_ocr_quality(alignment_data):
    ...
    for doc in alignment_data:
        try:
            ...
            # Optionally, print more details about the document with missing key
...
def generate_result_visualizations(metrics_df, summary_df):
    ...
    try:
        ...
        # Fallback for different versions of seaborn/matplotlib
        try:
    ...
    try:
        # 1. Document type accuracy comparison
    ...
    try:
        ...
        for i, v in enumerate(summary_df['count']):
            plt.text(i, v + 0.5, str(int(v)), ha='center')  # Ensuring integer values
        plt.tight_layout()
    ...
    try:
        # 4. Correlation between word count and accuracy
    ...
    try:
        # 5. Page number impact on accuracy
        # Check if page_number exists and has multiple values
        if 'page_number' in metrics_df.columns and metrics_df['page_number'].nunique() > 1:
            ...
            # Sort by page number to ensure proper display order
    ...
    try:
        ...
        with open(report_path, 'w') as f:
            ...
            # Check if these variables exist before using them
            try:
            ...
            # Find best and worst performing document types safely
        ...
        for page in doc_alignments:
            try:
                ...
                if not page_metrics.empty:
                    ...
                    print(f"  - Page {page['page_number']}: {page['word_count']} words, Est. Accuracy: {page_accuracy:.2f}%")
                else:
                    print(f"  - Page {page['page_number']}: {page['word_count']} words, Est. Accuracy: No data")
...
# Execute analysis for a sample document
try:
...
print(f"Augmented Image Sets: {len('/content/augmented_images')}")
