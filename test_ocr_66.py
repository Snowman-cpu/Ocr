...
Original file is located at
    https://colab.research.google.com/drive/11czJ3ExYe0RGCsU9D3c1CCwogJq8FeFz
...
def fine_tune_trocr_model(train_loader, val_loader, output_dir, num_epochs=3):
    """
    Fine-tune a TrOCR model on custom data with detailed epoch progress tracking
    ...
    # Create a custom callback to track and display metrics
    class MetricsCallback(TensorBoardCallback):
        ...
        def on_evaluate(self, args, state, control, metrics=None, **kwargs):
            # Call the parent class to ensure proper TensorBoard logging
            ...
            if metrics:
                ...
                eval_metrics = {
                    ...
                    'eval_cer': metrics.get('eval_cer', float('nan')),
                    'eval_accuracy': metrics.get('eval_accuracy', float('nan')),
                }

                # Calculate WER if available in metrics
                if 'eval_wer' in metrics:
                    eval_metrics['eval_wer'] = metrics['eval_wer']

                # Add to epoch metrics list
                self.epoch_metrics.append(eval_metrics)

                # Create and display the progress table
                self._display_progress_table()

        def _display_progress_table(self):
            """Create and display a formatted progress table with the current metrics"""
            if self.epoch_metrics:
                # Create DataFrame from metrics
                metrics_df = pd.DataFrame(self.epoch_metrics)

                # Format the metrics for display (round to 4 decimal places)
                for col in metrics_df.columns:
                    if col != 'epoch':
                        metrics_df[col] = metrics_df[col].apply(lambda x: f"{x:.4f}" if pd.notnull(x) else "N/A")

                # Display the table
                display(HTML(f"<h3>Training Progress</h3>"))
                display(HTML(metrics_df.to_html(index=False)))

                print("\n" + "="*80)  # Add separator for better visibility
    ...
    # Set token IDs in all required places
    ...
    print(f"  - bos_token_id (decoder_start_token_id): {decoder_start_token_id}")
    ...
    # Set in encoder config if it exists
    if hasattr(model, 'encoder') and hasattr(model.encoder, 'config'):
        model.encoder.config.pad_token_id = pad_token_id

    # Set in decoder config if it exists
    if hasattr(model, 'decoder') and hasattr(model.decoder, 'config'):
        model.decoder.config.pad_token_id = pad_token_id
        model.decoder.config.decoder_start_token_id = decoder_start_token_id
        model.decoder.config.eos_token_id = eos_token_id
        model.decoder.config.forced_eos_token_id = eos_token_id
        # Ensure bos token is used
        model.decoder.config.use_bos_token = True
        model.decoder.config.use_eos_token = True
    ...
    # Create Seq2Seq trainer with our custom callback
    trainer = Seq2SeqTrainer(
        ...
        data_collator=lambda examples: {'pixel_values': torch.stack([example['pixel_values'] for example in examples]),
                                       'labels': torch.tensor([example['labels'] for example in examples])}
    )

    # Train the model
    print(f"\nFine-tuning TrOCR model for {num_epochs} epochs...")
    print("Each epoch will display a progress table with metrics.")
    print("=" * 80)
    ...
    if hasattr(metrics_callback, 'epoch_metrics') and metrics_callback.epoch_metrics:
        final_metrics = pd.DataFrame(metrics_callback.epoch_metrics)
        display(HTML("<h3>Training Complete - Final Metrics</h3>"))
        display(HTML(final_metrics.to_html(index=False)))
...
# Complete ready-to-use code for creating accurate transcriptions
# Just copy and paste this entire cell into your Colab notebook
...
# Path to processed images - this should work with your existing directory
processed_dir = '/content/ocr_data/processed_images'

# List all document directories in your processed_images folder
print("Available document directories:")
doc_dirs = [d for d in os.listdir(processed_dir) if os.path.isdir(os.path.join(processed_dir, d))]
for i, doc in enumerate(doc_dirs):
    print(f"{i+1}. {doc}")

# Function to create and save transcriptions
def create_transcription_file(doc_name):
    transcription_path = f'/content/transcriptions/{doc_name}.txt'
    ...
    if os.path.exists(transcription_path):
        ...
        print(f"Existing transcription for {doc_name}:")
        print("=" * 50)
        print(existing_text)
        print("=" * 50)
    ...
    <div style="background-color: #f8f9fa; padding: 10px; border-radius: 5px; margin-bottom: 10px;">
      ...
      <ol>
        <li>Look at the image above</li>
        <li>Type the text exactly as it appears (keeping original spelling, punctuation)</li>
        <li>Maintain paragraph breaks</li>
        <li>Only transcribe text from the first page or first few paragraphs</li>
        <li>Click "Save Transcription" when done</li>
      </ol>
    </div>
    """))

    # Handle any special characters in the existing text for JavaScript
    if existing_text:
        existing_text = existing_text.replace('\\', '\\\\').replace('`', '\\`').replace('$', '\\$')

    # Create a textarea for transcription
    display(HTML(
        f"""
        <textarea id="transcription" style="width: 100%; height: 300px; font-family: monospace;">{existing_text}</textarea>
        <button id="save_button" style="padding: 8px 16px; background-color: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer; margin-top: 10px;">Save Transcription</button>
        <script>
            document.getElementById('save_button').onclick = function() {{
                var text = document.getElementById('transcription').value;
                google.colab.kernel.invokeFunction('notebook.save_transcription', [text, '{doc_name}'], {{}});
            }};
        </script>
        """
    ))

    # Register function to save the transcription
    def save_transcription(text, doc):
        with open(f'/content/transcriptions/{doc}.txt', 'w', encoding='utf-8') as f:
            f.write(text)
        print(f"✅ Transcription saved for {doc}")

    register_callback('notebook.save_transcription', save_transcription)

# Function to display an image from a document and create transcription
def transcribe_document(doc_dir, page_index=0):
    ...
    images = sorted([f for f in os.listdir(doc_path) if f.endswith('.jpg') or f.endswith('.png')])
    if not images:
        print(f"No images found in {doc_path}")
        return

    if page_index >= len(images):
        print(f"Error: Page index {page_index} is out of range. Document has {len(images)} pages.")
        return

    # Get the selected image
    img_path = os.path.join(doc_path, images[page_index])

    # Display the image
    plt.figure(figsize=(12, 16))
    img = Image.open(img_path)
    plt.imshow(img)
    plt.title(f"Document: {doc_dir} - Page {page_index+1}")
    plt.axis('off')
    plt.show()

    # Create transcription
    create_transcription_file(doc_dir)

# Let user select which document to transcribe
selected_doc = input("Enter the number of the document you want to transcribe: ")
try:
    doc_index = int(selected_doc) - 1
    if 0 <= doc_index < len(doc_dirs):
        selected_doc_dir = doc_dirs[doc_index]

        # Ask which page to transcribe
        page_input = input(f"Which page of {selected_doc_dir} do you want to transcribe? (1, 2, 3, etc.): ")
        page_index = int(page_input) - 1

        # Display the image and create transcription
        transcribe_document(selected_doc_dir, page_index)
    else:
        print(f"Invalid selection. Please enter a number between 1 and {len(doc_dirs)}")
except ValueError:
    print("Please enter a valid number")

# After creating transcriptions, you can retrain with more epochs:
print("\nAfter creating transcriptions for a few documents, run your training with 10 epochs:")
print("max_pages = 6")
print("evaluate = True")
print("fine_tune = True")
print("num_epochs = 10")
print("\nresults = run_ocr_pipeline(")
print("    max_pages_per_pdf=max_pages,")
print("    evaluate_model=evaluate,")
